...
package binary404.autotech.gnu.trove.impl.hash;
...
abstract public class THash implements Externalizable {
    ...
    protected static final float DEFAULT_LOAD_FACTOR = gnu.trove.impl.Constants.DEFAULT_LOAD_FACTOR;
     ...
     * 11.
     */
    protected static final int DEFAULT_CAPACITY = gnu.trove.impl.Constants.DEFAULT_CAPACITY;
    ...
    public THash( int initialCapacity, float loadFactor ) {
        ...
        setUp( gnu.trove.impl.HashFunctions.fastCeil( initialCapacity / loadFactor ) );
    }
    ...
    public void ensureCapacity( int desiredCapacity ) {
        if ( desiredCapacity > ( _maxSize - size() ) ) {
			rehash( gnu.trove.impl.PrimeFinder.nextPrime( Math.max( size() + 1,
				gnu.trove.impl.HashFunctions.fastCeil( ( desiredCapacity + size() ) / _loadFactor ) + 1 ) ) );
    ...
    public void compact() {
        ...
        rehash( gnu.trove.impl.PrimeFinder.nextPrime( Math.max( _size + 1,
	        gnu.trove.impl.HashFunctions.fastCeil( size() / _loadFactor ) + 1 ) ) );
        computeMaxSize( capacity() );
    ...
    protected int setUp( int initialCapacity ) {
        ...
        capacity = gnu.trove.impl.PrimeFinder.nextPrime( initialCapacity );
        computeMaxSize( capacity );
    ...
    protected final void postInsertHook( boolean usedFreeSlot ) {
        ...
        if ( ++_size > _maxSize || _free == 0 ) {
            ...
            int newCapacity = _size > _maxSize ? gnu.trove.impl.PrimeFinder.nextPrime( capacity() << 1 ) : capacity();
            rehash( newCapacity );
            computeMaxSize( capacity() );
        }
    }
