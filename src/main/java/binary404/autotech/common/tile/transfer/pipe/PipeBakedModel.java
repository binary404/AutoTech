...
public class PipeBakedModel implements IBakedModel {
    ...
    public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side, @Nonnull Random rand, @Nonnull IModelData extraData) {
        PipeState pipeState = new PipeState(state, extraData.getData(TilePipe.ATTACHMENTS_PROPERTY), side, rand);
    ...
    private List<BakedQuad> createQuads(PipeState state) {
        ...
        if (state.getState() != null) {
            boolean north = state.getState().get(BlockPipe.NORTH);
            boolean east = state.getState().get(BlockPipe.EAST);
            boolean south = state.getState().get(BlockPipe.SOUTH);
            boolean west = state.getState().get(BlockPipe.WEST);
            boolean up = state.getState().get(BlockPipe.UP);
            boolean down = state.getState().get(BlockPipe.DOWN);
        ...
        if (state.getState() != null) {
            boolean invNorth = state.getState().get(BlockPipe.INV_NORTH);
            boolean invEast = state.getState().get(BlockPipe.INV_EAST);
            boolean invSouth = state.getState().get(BlockPipe.INV_SOUTH);
            boolean invWest = state.getState().get(BlockPipe.INV_WEST);
            boolean invUp = state.getState().get(BlockPipe.INV_UP);
            boolean invDown = state.getState().get(BlockPipe.INV_DOWN);
