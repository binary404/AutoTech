...
public class EnergyProxy {
    ...
    public Set<TileCable> all(World world) {
        final Set<TileCable> cableTiles = new HashSet<>();
        Iterator<BlockPos> iterator = this.cables.iterator();
        while (iterator.hasNext()) {
            BlockPos pos = iterator.next();
            TileEntity tileEntity = world.getTileEntity(pos);
            if (tileEntity instanceof TileCable) {
                cableTiles.add((TileCable) tileEntity);
            } else {
                iterator.remove();
            }
        }
    ...
    public List<BlockPos> search(Block block, TileCable tile, Direction side) {
